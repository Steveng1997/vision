...
export class ManagerComponent implements OnInit {
  ...
  GetManager() {
    this.service.getByEncargada(this.liquidationManager.encargada).subscribe((rp: any) => {
      if (rp.length > 0) this.existIdManager = true
      else this.existIdManager = false
      return true
    })
    return false
  }
  ...
  calculateServices(): any {
    if (this.liquidationManager.encargada) {
      ...
      setTimeout(() => {
        if (!this.getThoseThatNotLiquidated()) return
      }, 800);
  ...
  inputDateAndTime() {
    setTimeout(() => {
      if (!this.sumTotalWithDate()) return
    }, 3000);

    setTimeout(() => {
      if (!this.comissionWithDate()) return
    }, 5000);

    setTimeout(() => {
      if (!this.fixedDayWithDate()) return
    }, 8000);
